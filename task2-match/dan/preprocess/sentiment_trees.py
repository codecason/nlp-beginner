...
def process_trees(wmap):
    for split in ['train', 'dev', 'test']:
        ...
        print(len(trees))
        pickle.dump(trees, open('../data/sentiment/' + split + '_alltrees', 'wb'), protocol=pickle.HIGHEST_PROTOCOL)
...
if __name__=='__main__':
    ...
    process_trees(wmap)
    train = pickle.load(open('../data/sentiment/train_alltrees', 'rb'))
    dev = pickle.load(open('../data/sentiment/dev_alltrees', 'rb'))
    test = pickle.load(open('../data/sentiment/test_alltrees', 'rb'))
    ...
    for sent, label in t_sents:
        c[label] += 1
    print(c)
    pickle.dump(t_sents, open('../data/sentiment/train-rootfine', 'wb'))
    ...
    for sent, label in dev_sents:
        c[label] += 1
    print(c)
    pickle.dump(dev_sents, open('../data/sentiment/dev-rootfine', 'wb'))
    ...
    for sent, label in test_sents:
        c[label] += 1
    print(c)
    pickle.dump(test_sents, open('../data/sentiment/test-rootfine', 'wb'))
