...
parameters[
    'zeros'] = True  # Boolean variable to control replacement of  all digits by 0
parameters['char_dim'] = 30  #Char embedding dimension
parameters['word_dim'] = 100  #Token embedding dimension
parameters['word_lstm_dim'] = 200  #Token LSTM hidden layer size
parameters['word_bidirect'] = True  #Use a bidirectional LSTM for words
parameters[
    'embedding_path'] = "./data/glove.6B.100d.txt"  #Location of pretrained embeddings
...
def prepare_dataset(sentences, word_to_id, char_to_id, tag_to_id, lower=False):
    ...
    for s in sentences:
        ...
        words = [
            word_to_id[lower_case(w, lower) if lower_case(w, lower) in
...
def init_embedding(input_embedding):
    ...
    """
    print("input_embedding size", input_embedding.shape)
...
def init_linear(input_linear: nn.Linear):
    ...
    """
    print("input_linear size", input_linear.weight.size())
    bias = np.sqrt(6.0 /
                   (input_linear.weight.size(0) + input_linear.weight.size(1)))
...
def init_lstm(input_lstm):
    ...
    if input_lstm.bidirectional:
        for ind in range(0, input_lstm.num_layers):
            ...
            sampling_range = np.sqrt(6.0 /
                                     (weight.size(0) / 4 + weight.size(1)))
            nn.init.uniform(weight, -sampling_range, sampling_range)
...
class BiLSTM_CRF(nn.Module):
    def __init__(self,
        ...
        if char_embedding_dim is not None:
            ...
            init_embedding(self.char_embeds.weight
                           )  # ++ self.char_embeds.weight.size() = (75, 25)
        ...
        self.word_embeds = nn.Embedding(vocab_size,
                                        embedding_dim)  # ++ vocab_size= 17943
        if pre_word_embeds is not None:
            #Initializes the word embeddings with pretrained word embeddings
            self.pre_word_embeds = True
            self.word_embeds.weight = nn.Parameter(
                torch.FloatTensor(pre_word_embeds))  ## ++ here add parameters
        else:
    ...
    def score_sentences(self, feats, tags):
        ...
        score = torch.sum(self.transitions[pad_stop_tags,
                                        pad_start_tags]) + torch.sum(
                                            feats[r, tags])
        return score
    ...
    def forward_alg(self, feats):
        ...
        for feat in feats:
            ...
            forward_var = max_tag_var + torch.log(
                torch.sum(torch.exp(tag_var), dim=1)).view(1, -1)  # ).view(1, -1)
        terminal_var = (forward_var +
                        self.transitions[self.tag_to_ix[STOP_TAG]]).view(1, -1)
    ...
    def viterbi_algo(self, feats):
        ...
        for feat in feats:
            ...
            viterbivars_t = next_tag_var[range(len(
                bptrs_t)), bptrs_t]  # holds the viterbi variables for this step
    ...
    def get_lstm_features(self, sentence, chars2, chars2_length, d):
        ...
        if self.char_mode == 'CNN':
            ...
            chars_embeds = nn.functional.max_pool2d(
                chars_cnn_out3, kernel_size=(chars_cnn_out3.size(2),
                                            1)).view(chars_cnn_out3.size(0),
                                                    self.out_channels)
...
if not parameters['reload']:
    ...
    for epoch in range(1, number_of_epochs):
        for i, index in enumerate(np.random.permutation(len(train_data))):
            ...
            # loss += neg_log_likelihood.data[0].item() / len(data['words'])
            ...
            #Evaluating on Train, Test, Dev Sets # ++ eval_every = len(train_data)
...
for data in final_test_data:
    ...
    # We are getting the predicted output from our model
    if use_gpu:
        val, predicted_id = model(dwords.cuda(), chars2_mask.cuda(),
                                  chars2_length, d)
    else:
